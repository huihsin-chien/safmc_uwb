...
class UWBData:
    ...
    def __init__(self, distance: float, RX_power: float):
...
# 這幾個類別用來儲存他們的位置
# 由於 mediatorUWB & publishUWB 不需要知道其在場上的座標，因此不記錄於 UWBDevice 
class UWBDevice:
    eui: str = ""
...
class UWBDataMatrix:
    ...
    # 將測量資訊加入資料庫
    def add_measurement(self, tag_eui: str, anchor_eui: str, distance: float, RX_power: float) -> None:
    ...
    def get_distance(self, tag_eui: str, anchor_eui: str) -> Optional[float]:
        ...
        # 線性修正固定偏差值 & 縮放比例，來提高精準度 TODO 確認公式正確性
        ...
        # 若要減少計算時間，可以註解掉上面的去除極值的計算，直接輸出中位數
        # distances.sort()
        # return distances[len(distances) // 2]
...
class UWBPublisher():
    ...
    def __init__(self):
        ...
        # 開啟 Ports 用以讀取 mediatorUWB 及 publishUWB
        ...
        # 用於儲存估計各 tag 位置
        self.uwb_data_matrix = UWBDataMatrix(time_threshold=0.2, anchors=self.anchors, tags=self.tags + self.anchors[4:])

        # 定期更新 Serial & 透過 USB Serial 讀取 UWB 裝置距離 & 發佈 Tag 的位置
        self.update_serial_loop = self.create_timer(2, self.update_serial_list)                                 # 檢測 USB 硬體更改
        self.read_serial_loop = self.create_timer(0.05, lambda: self.read_serial(self.uwb_data_matrix))         # 讀取所有 USB
        ...
        self.self_calibration_loop = self.create_timer(1, lambda: self.self_calibration(self.uwb_data_matrix))
    ...
    def self_calibration(self, uwb_data_matrix) -> None:
        ...
        for eui in self.is_in_anchor_state.keys():
            if self.is_in_anchor_state[eui]:
                continue
            self.read_serial(uwb_data_matrix)
            if sum(1 for anchor in self.anchors if self.build_coord.have_enough_data_between([eui], [anchor.eui])) >= 4:
                coord = uwb_data_matrix.locate_tag(eui, tol=1e-6) # 在建立座標系時，使用較小的 tolerance 以獲得較佳結果
                if coord is not None and all(coord):
                    ...
                    self.target_state += self.turn_into_anchor_symbol[eui]
            ...
            for line in lines: # 逐一讀取每一行資料
                ...
                if line.startswith("anchor_range,"): # 對於距離資料，新增測距結果到 UWB Data Matrix
                    try: 
                        ...
                        uwb_data_matrix.add_measurement(from_eui, to_eui, float(distance), RX_power)
