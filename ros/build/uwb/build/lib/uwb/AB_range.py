...
class UWBPublisher():
    ...
    def build_coord(self):
        ...
        def have_enough_data_between(tag_euis: list[str], anchor_euis: list[str]) -> bool:
            ...
            return all(
                len(uwb_calibration_data_matrix.data[tag_eui][anchor_eui]) > 180
        ...
        dbg("- - distance matrix: ", distance_matrix)
        distances = [distance_matrix[0][1], distance_matrix[0][2], distance_matrix[0][3], distance_matrix[1][2], distance_matrix[1][3], distance_matrix[2][3]]
        dbg("- - distances: ", ",".join([str(d) for d in distances]))
        dbg("- - review distances: ", ",".join([f"{(d - 0.1766) / 1.0349}" for d in distances]))
