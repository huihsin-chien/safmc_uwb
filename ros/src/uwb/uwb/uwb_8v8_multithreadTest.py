...
class UWBPublisher(Node):
    ...
    def __init__(self):
        ...
        # 為每一個 Tag 建立一個 Process 來發佈其位置
        for tag_eui in self.uwb_data_matrix.tags.keys():
            Process(target=lambda: self.multi_thread_publish_tag_position(tag_eui)).start()
    ...
    def multi_thread_publish_tag_position(self, tag_eui) -> None:
        # rclpy.init()
        per_tag_publisher = UWBPerTagPublisher(tag_eui, self.uwb_data_matrix)
        try: # 試圖保持程式運行。如果程式被強制終止，以 finally 正確結束程式
            rclpy.spin(per_tag_publisher)
        except KeyboardInterrupt:
            pass
        finally:
            per_tag_publisher.destroy_node()
            rclpy.shutdown()

class UWBPerTagPublisher(Node):
    def __init__(self, tag_eui, uwb_data_matrix):
        super().__init__(f'position_publisher_{tag_eui.replace(":", "")}')
        self.tag_eui = tag_eui
        self.uwb_data_matrix = uwb_data_matrix
    ...
    def publish_tag_position(self):
        coordinate = self.uwb_data_matrix.locate_tag(self.tag_eui)
        ...
        msg = TagPosition()
        msg.eui = self.tag_eui
        msg.x, msg.y, msg.z = coordinate
        msg.timestamp = time.time_ns()
        self.tag_position_publisher.publish(msg)

        dbg(f"- coordinate for {self.tag_eui} is {coordinate}")
